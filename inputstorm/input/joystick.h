...
struct joystick {
  ...
  static unsigned int constexpr max_button_action = static_cast<int>(key::action::REPEAT);   // there's no repeat action for joystick buttons
...
private:
  ...
  std::array<std::array<key::action, max_button>, max> button_state;            // last pressed or released states
